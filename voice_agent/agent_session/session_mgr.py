...
class SessionMgr:
    ...
    async def input_audio_data(
        ...
        Decodes base64-encoded PCM audio data and sends it to the appropriate
        agent session for processing.
        
        Args:
            room_id (str): Room identifier
            user_id (str): User identifier
            pcm_base64 (str): Base64-encoded PCM audio data
        ...
        try:
            ...
            self.log.debug(f"Input audio data to {room_id}_{user_id}: {len(pcm_data)} bytes")
            
            # Pass audio data to session
            await session.handle_audio_data(pcm_data)
